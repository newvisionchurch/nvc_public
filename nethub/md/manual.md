...
탭: AP Status | EFG Remote | Log Export | 동기화 | 설정
```
...
| 경고 설정 | 경고 표시 임계값 설정 |
| 컬럼 설정 | 표시 컬럼 선택 |
| ELK Stuck Count | 날짜 범위별 Stuck 이벤트 집계 |
...
| 분류 / 점수 | ResetScore 결과 |
| ELK Stuck | 로컬 JSONL 캐시 기반 Stuck 이벤트 수 |
...
5. 실행 결과는 `runtime/operations.log`와 GitHub 이력에 기록됩니다.
...
| 서브탭 | 내용 |
|--------|------|
| 일반 | 폰트, AP 등록 수, 정렬, 임계값, 색상 |
| 자동화 | 시작 시 자동 조회, 집계, 정렬, 진단 |
| 보안 | 로컬 경로, Keychain, GitHub PAT, UniFi API Key |
| 사용자 | 계정, 역할, 권한, TOTP, 개발자 PC |

사용자 관리 탭은 `can_manage_users` 권한이 있는 계정에만 표시됩니다.
...
| ELK Stuck이 모두 0 | 로그 동기화 미실행 | 동기화 탭에서 NAS ELK Sync 실행 |
| AP Reset 버튼 비활성 | 권한 없음 | 관리자에게 권한 요청 |
| 게시판 오류 | GitHub PAT 만료 또는 권한 부족 | PAT 재입력 또는 권한 확인 |
| GitHub 연결 실패 | PAT 만료, 네트워크 문제 | 설정 탭에서 PAT 재등록 |
